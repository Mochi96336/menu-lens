...
The interface keeps price, serving, and preparation in one shallow axonometric volume. Price remains continuous; serving and preparation retain their explicit semantic bands. Three mutually perpendicular semantic matrix planes divide the volume into readable regions without replacing the products' exact coordinates. The price × serving, price × preparation, and serving × preparation controls rotate the same volume so the omitted axis becomes shallow depth rather than disappearing. Quaternion interpolation moves the 30 persistent product nodes, the matrix planes, the bounding frame, and all three axes along one continuous rotation instead of cross-fading between independent plots.

Tapping a node focuses its cell on the currently readable matrix plane and expands that cell in place into a compact list of dish names and prices. Changing the observation plane preserves the anchor dish, then recomputes its semantic cell on the newly visible pair of axes. This makes coordinate collisions readable without adding a separate menu, transaction, or cart flow.
