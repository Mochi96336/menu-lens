...
const evidencePath = new URL("../docs/research-history/reviews/06-return-continuity/layout-checks.json", import.meta.url);
const screenshotDirectory = process.env.MENU_LENS_LAYOUT_SCREENSHOTS ?? "/tmp/menu-lens-06-return-layout";
const viewports = [
  { key: "320", width: 320 },
  { key: "390", width: 390 },
...
await mkdir(new URL("../docs/research-history/reviews/06-return-continuity/", import.meta.url), { recursive: true });
...
const measure = (state) => {
  ...
  const visible = Boolean(resetRect && resetRect.bottom > 0 && resetRect.top < window.innerHeight);
  ...
  return {
    ...
    uniqueProducts: new Set(products.map((product) => product.dataset.productId)).size,
...
try {
  for (const viewport of viewports) {
    ...
    await lastProduct.locator("summary").click();
    ...
    for (const state of [initial, focused, deep, returned]) {
      if (state.categories !== 6 || state.products !== 30 || state.uniqueProducts !== 30) failures.push(`${state.state}:fixture`);
      if (state.documentOverflowX || state.frameOverflowX || state.screenOverflowX || state.topbarOverflowX || state.labelOverflowX || state.resetOverflowX) {
        failures.push(`${state.state}:horizontal-overflow`);
      }
      if (!state.topbarInsideFrame) failures.push(`${state.state}:topbar-outside-frame`);
    }
    if (focused.topbarPosition !== "sticky" || !focused.resetVisible || focused.topbarTop < -1 || focused.topbarTop > 1) failures.push("focused:sticky-not-visible");
    if (deep.topbarPosition !== "sticky" || !deep.resetVisible || deep.topbarTop < -1 || deep.topbarTop > 1) failures.push("deep:sticky-not-visible");
    if (focused.topbarHeight > 48 || deep.topbarHeight > 48) failures.push("focused:topbar-too-tall");
...
if (failures.length) {
  throw new Error(`A-M3 layout validation failed: ${JSON.stringify(failures)}`);
}

console.log("A-M3 return layout validation passed.");
