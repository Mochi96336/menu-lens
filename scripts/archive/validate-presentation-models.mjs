...
for (const [objectId, note] of Object.entries(presentationNotes)) {
  ...
  for (const field of ["shortLabel", "variable", "before", "after", "unchanged"]) {
    requireString(note[field], `Presentation note ${objectId}.${field}`);
  }
}

console.log(`Design models: ${designModels.length} models, ${primaryOwners.size} grouped objects, ${Object.keys(presentationNotes).length} explicit notes.`);
