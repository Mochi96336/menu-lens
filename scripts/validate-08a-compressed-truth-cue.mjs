...
const child = registrySandbox.window.menuLensPrototypeRegistry.prototypes.find((prototype) => prototype.id === "08A");
if (!child) throw new Error("Prototype registry must contain 08A Compressed Truth Cue.");
if (child.parentId !== "08" || child.family !== "horizontal") {
  throw new Error("08A must remain a horizontal child of 08.");
}
if (child.path !== childPath || child.validationProfile !== "spread-truth-cue") {
  throw new Error("08A must retain its registered path and truth-cue validation profile.");
}
...
if (child.assets.styles.join("|") !== expectedStyles.join("|") || child.assets.scripts.join("|") !== expectedScripts.join("|")) {
  throw new Error("08A must reuse the 08 assets and add only compressed-truth-cue.css.");
}
...
for (const bannedProperty of ["flex-basis", "scroll-snap", "overflow-x", "translate", "transform", "position: sticky", "font-size"]) {
  if (childCss.includes(bannedProperty)) {
    throw new Error(`08A must not change geometry, camera, typography, or navigation: ${bannedProperty}`);
  }
}
...
if (markGroups.length !== menu.categories.length) {
  throw new Error(`08A requires one truth-cue group per category; received ${markGroups.length}.`);
}
const markCount = markGroups.reduce((sum, match) => sum + [...match[1].matchAll(/<span\b/g)].length, 0);
if (markCount !== menu.products.length) {
  throw new Error(`08A requires one existing mark per product; received ${markCount}.`);
}
...
if (childHtml.includes("選這道") || childHtml.includes("加入購物車")) {
  throw new Error("08A must remain a menu-reading study without an order action.");
}

console.log("08A validation passed: exact 08 controller, 6 fixed categories, 30 product-derived compressed truth marks, no second mechanism.");
