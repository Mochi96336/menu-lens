import { createCandidateComparisonModel, isComparisonSelected, } from "../customer/candidate-comparison.js";
import { metadataConfidenceLabels, metadataSourceLabels, } from "../customer/menu-semantic-labels.js";
...
const evidenceText = (evidence) => evidence.valueLabel ?? "未提供";
const evidenceSupport = (evidence) => {
...
const selectionStatusText = (model) => {
    if (model.candidates.length < 2)
        return "至少需要 2 道考慮項目才能比較";
    const count = model.selectedProducts.length;
    if (count === 0)
        return "尚未選擇 · 最多 3 道";
    if (count === 1)
        return "已選 1 / 3 道 · 再選 1 道即可比較";
    return `已選 ${count} / 3 道`;
};
export const createCandidateComparison = (menu, onBack, onToggleSelection) => {
    ...
    const backButton = element("button", "candidate-comparison__back", "回到考慮項目");
    backButton.type = "button";
    const eyebrow = element("p", "eyebrow", "考慮項目比較");
    const heading = element("h2", "candidate-comparison__title", "比較考慮項目");
    heading.tabIndex = -1;
    const hint = element("p", "candidate-comparison__hint", "選擇 2–3 道；只是在比較，尚未點餐。");
    ...
    let renderedCandidates = null;
    let renderedComparison = null;
    const renderSelectors = (candidates) => {
        ...
        menu.products.filter((product) => membership.has(product.id)).forEach((product) => {
            ...
            const meta = element("p", "candidate-comparison__selector-meta", [category?.name, product.availability === "sold_out" ? "已售完" : null]
                .filter((entry) => Boolean(entry))
                .join(" · "));
            copy.append(name, meta);
            const button = element("button", "candidate-comparison__selector-action", "比較");
            button.type = "button";
    ...
    const resetStatus = (candidates, comparison) => {
        status.textContent = selectionStatusText(createCandidateComparisonModel(menu, candidates, comparison));
    };
    ...
    const render = (candidates, comparison) => {
        ...
        model.dimensions.forEach((dimension) => {
            ...
            dimension.values.forEach((value) => {
                ...
                if (support)
                    valueCell.append(element("small", "candidate-comparison__value-support", support));
