import { beginAnchorSelection, cancelAnchorSelection, categoryScrollBehavior, clearAnchor, createCompleteMenuModel, createInitialMenuReadingState, focusCategory, isAnchorSelectionCancelKey, selectAnchor, setActiveCategory, setSemanticAxis, showAllCategories, showMenuOverview, } from "../customer/menu-reading.js";
...
export const mountMenuApp = (root, menu) => {
    ...
    let overview;
    const render = () => overview.render(state);
    ...
    const selectCategory = (categoryId) => {
        const isSameFocusedCategory = state.expansion.kind === "category" && state.expansion.categoryId === categoryId;
    ...
    overview = createMenuOverview(model, selectCategory, beginAnchor, cancelAnchor, chooseAnchor, clearCurrentAnchor, chooseSemanticAxis, showOverviewFromContext, showAll);
    ...
    document.addEventListener("keydown", (event) => {
        if (state.anchorReading.kind !== "selecting" || !isAnchorSelectionCancelKey(event.key))
            return;
