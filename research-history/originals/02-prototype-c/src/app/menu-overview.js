...
export const createMenuOverview = (model, onSelectCategory, onBeginAnchorSelection, onCancelAnchorSelection, onSelectAnchor, onClearAnchor, onSelectAxis, onShowOverview, onShowAll) => {
    ...
    intro.append(element("p", "eyebrow", "菜單全貌"), title, element("p", "menu-map__hint", "先看每區的份量、價位與代表菜名；點一區會在原位置展開，其他區域仍留在整張菜單裡。"));
    const stack = element("div", "category-stack");
    const sections = model.categories.map((category, index) => createMenuCategorySection(category, index, onSelectCategory, onBeginAnchorSelection, onCancelAnchorSelection, onSelectAnchor, onClearAnchor, onSelectAxis));
    ...
    const render = (state) => {
        ...
        sections.forEach((section) => {
            ...
            section.setState(categoryIsExpanded(state, section.categoryId), isCurrent, state.expansion.kind === "category" && isCurrent
                ...
                ? state.semanticAxis
                : null, state.expansion.kind === "category" && isCurrent);
        });
    };
    return {
        element: root,
