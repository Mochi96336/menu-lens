...
export const modelDiagramPresentations = Object.freeze({
  ...
  "horizontal-navigation": freezeModelPresentation({
    ...
    statement: "比較分類展寬、無捲動加權橫帶、料理長軸與局部焦點的不同橫向單位。",
    ...
    edges: [
      ...
      ["spread", "weighted-strip"],
      ["weighted-strip", "ribbon"],
      ["ribbon", "fisheye"],
    ],
    sections: {
      "market-baseline": {
      ...
      "weighted-strip": {
        label: "加權橫帶",
        conceptLabel: "Count-weighted viewport",
        note: "內容比例 · 無長距離",
        position: { x: 50, y: 50 },
        vignette: { type: "horizontal", variant: "spread", activeIndex: 2, expansion: 1.55 },
      },
      ribbon: {
        label: "料理 Ribbon",
  ...
  "paper-field": freezeModelPresentation({
    ...
    statement: "固定二維紙面是共同場域；單一局部鏡頭與二維彈性幾何分別測試閱讀邊界。",
    motif: "paper-field",
    routeLayout: {
      type: "balanced-rail",
  ...
  "multiscale-focus": freezeModelPresentation({
    ...
    statement: "同一個分類 focus 問題分成可信回程修正，以及以連續摺頁表達相鄰關係的替代拓撲。",
    ...
    edges: [["model", "necessary-corrections"], ["model", "folded-topology"]],
    sections: {
      model: {
        label: "尺度模型",
      ...
      "folded-topology": {
        label: "摺頁拓撲",
        conceptLabel: "Continuous folded adjacency",
        note: "連續摺頁 · 相鄰壓縮",
        position: { x: 72, y: 76 },
        vignette: { type: "scale", variant: "focus" },
      },
