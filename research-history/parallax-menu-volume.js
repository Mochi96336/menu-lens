(() => {
  ...
  const updateGuidance = (current, alignment) => {
    ...
    else if (!state.hasPinched) gestureHint.textContent = "兩指張合，調整排版深度";
    else gestureHint.textContent = "拖曳轉向 · 點料理看資訊";
  ...
  stage.addEventListener("pointerdown", (event) => {
    if (event.target.closest(".parallax-reset, .parallax-detail")) {
      gestureMoved = false;
      return;
    }
