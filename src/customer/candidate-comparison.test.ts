...
const candidates: CandidateState = { productIds: ["p3", "stale", "p1", "p2", "p4", "p2"] };
...
test("comparison state begins empty and identity-only", () => {
  ...
  ["quantity", "configuration", "winner", "score", "rank", "order"].forEach((term) =>
    assert(!JSON.stringify(state).toLowerCase().includes(term), `${term} must not enter comparison state`),
  );
});
...
if (failures > 0) throw new Error(`${failures} Candidate comparison test(s) failed`);
